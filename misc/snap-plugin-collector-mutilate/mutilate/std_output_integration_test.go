...
func TestStdoutParser(t *testing.T) {
	Convey("Opening non-existing file should fail", t, func() {
		data, err := parseMutilateStdout("/non/existing/file")
	...
	Convey("Attempting to read file with wrong number of read columns should return"+
		...
		data, err := parseMutilateStdout(GetCurrentDirFilePath(
	...
	Convey("Attempting to read a file with no read row at all should return"+
		" an error and no metrics", t, func() {
		data, err := parseMutilateStdout(GetCurrentDirFilePath("/mutilate_missing_read_row.stdout"))

		So(data, ShouldHaveLength, 0)
		So(err.Error(), ShouldEqual, "No default mutilate statistics found")
	})

	Convey("Attempting to read a file with no swan-specific row at all"+
		" should return an error and no metrics", t, func() {
		data, err := parseMutilateStdout(GetCurrentDirFilePath("/mutilate_missing_swan_row.stdout"))

		So(data, ShouldHaveLength, 0)
		So(err.Error(), ShouldEqual, "No swan-specific statistics found")
	})

	Convey("Attempting to read a file with malformed swan-specific row should return"+
		" an error and no metrics", t, func() {
		data, err := parseMutilateStdout(GetCurrentDirFilePath("/mutilate_malformed_swan_row.stdout"))

		So(data, ShouldHaveLength, 0)
		So(err.Error(), ShouldEqual, "Swan-specific row malformed")
	})

	Convey("Attempting to read a file with swan-specific row missing metric"+
		" value should return an error and no metrics", t, func() {
		data, err := parseMutilateStdout(GetCurrentDirFilePath("/mutilate_missing_metric_in_swan_row.stdout"))

		So(data, ShouldHaveLength, 0)
		So(err.Error(), ShouldEqual, "Swan-specific row is missing metric value")
	})

	Convey("Attempting to read a file with swan-specific row missing percentile value"+
		" should return an error and no metrics", t, func() {
		data, err := parseMutilateStdout(GetCurrentDirFilePath("/mutilate_swan_row_missing_percentile_in_description.stdout"))

		So(data, ShouldHaveLength, 0)
		So(err.Error(), ShouldEqual, "Swan-specific row is missing percentile value")
	})

	Convey("Attempting to read a file with read row containing incorrect values should return"+
		" en error and no results", t, func() {
		data, err := parseMutilateStdout(GetCurrentDirFilePath("/mutilate_non_numeric_default_metric_value.stdout"))
...
func soMetric(mutilateMetric metric, name string, value float64) {
