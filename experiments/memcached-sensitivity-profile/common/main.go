...
// MeasurementContext ... TODO: make lint happy
type MeasurementContext struct {
	...
	TaskInfo          executor.TaskInfo
}

// Main ... TODO: make lint happy
func Main(
	measurementHook func(measurementContext MeasurementContext) (func(), error),
) {
...
It executes workloads and triggers gathering of certain metrics like latency (SLI) and the achieved number of Request per Second (QPS/RPS)`)
	...
	slo := sensitivity.SLOFlag.Value()
	if sensitivity.PeakLoadFlag.Value() == sensitivity.RunTuningPhase {
		load, err = GetPeakLoad(hpLauncher, loadGenerator, slo)
		if err != nil {
	...
	measurementContext := MeasurementContext{
		...
		// KubernetesAddress: // I cannot get that information for HPExecutor
		// KubernetesNode: // I cannot get that information for HPExecutor
	}
		...
		for loadPoint := 0; loadPoint < sensitivity.LoadPointsCountFlag.Value(); loadPoint++ {
			...
			for repetition := 0; repetition < sensitivity.RepetitionsFlag.Value(); repetition++ {
				...
				err := func() error {
					...
					// Intersection point and extra phase context.
					measurementContext.TaskInfo = hpHandle
					measurementContext.Tags = snapTags
					cleanupFunc, err := measurementHook(measurementContext)
					if err != nil {
						return errors.Wrapf(err, "Pre-measurement hook failed in %s, repetition %d.", phaseName, repetition)
					}
					defer cleanupFunc()
