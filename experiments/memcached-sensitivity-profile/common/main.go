...
func Main(
	preMeasurementHook func(measurementContext MeasurementContext, ti executor.TaskInfo) error,
	postMeasurementHook func() error,
) {
		...
		for loadPoint := 0; loadPoint < sensitivity.LoadPointsCountFlag.Value(); loadPoint++ {
			...
			for repetition := 0; repetition < sensitivity.RepetitionsFlag.Value(); repetition++ {
				...
				err := func() error {
					...
					// Intersection point.
					err = preMeasurementHook(measurementContext, hpHandle)
					if err != nil {
