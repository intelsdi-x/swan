...
func TestBackendWithMockedExecutor(t *testing.T) {
	...
	Convey("When using Backend launcher", t, func() {
		...
		expectedCommand := "java -jar -server -Djava.util.concurrent.ForkJoinPool.common.parallelism=8 -Xms10g -Xmx10g -XX:NativeMemoryTracking=summary -XX:+UseParallelGC -XX:ParallelGCThreads=8 -XX:ConcGCThreads=4 -XX:InitiatingHeapOccupancyPercent=80 -XX:MaxGCPauseMillis=100 -Dspecjbb.controller.host=127.0.0.1 test -m backend -G GRP1 -J specjbbbackend1 -p /usr/share/specjbb/config/specjbb2015.props"
